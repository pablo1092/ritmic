		...
		try {
			if (element.equals("music")) {
				...
				case "staffDef": 
					...
					if (staffKeySig == null) {
						if (lastKeySig == null) {
							throw new IM3RuntimeException("key.sig is null both in scoreDef and staffDeff");
						}
						...
						if (measureDuration == null) {
							if (!currentMeasure.getTime().isZero()) { // avoid first bar for anacrusis
								...
								if (maximumVoicesTime.isZero()) {
									throw new ImportException("Cannot infer the measure duration for mRest " + mrest);
								}
