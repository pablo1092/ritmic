...
import es.ua.dlsi.im3.util.Histogram;
...
public class MusicXMLSAXImporter extends XMLSAXImporter {
			...
			if (lastDuration != null) {
				if (tupletActualNotes != null) {
					...
					try {
						...
						Logger.getLogger(MusicXMLSAXImporter.class.getName()).log(Level.INFO, "Cannot get duration of duration {0}, is it a mRest?", lastDuration);
						lastFigure = null;
					}
				}
			} else {
		...
		for (Entry<StaffAndVoice, ArrayList<ITimedElementInStaff>> entry: measureElementsToInsert.entrySet()) {
			...
			for (ITimedElementInStaff element: entry.getValue()) {
				if (element instanceof Atom) {
					...
					if (atom.getDuration().getExactTime().equals(Figures.NO_DURATION.getDuration())) {
						if (countAtoms == 1) {
							if (atom instanceof SimpleRest) {
								...
								Logger.getLogger(MusicXMLSAXImporter.class.getName()).log(Level.INFO, "Replacing rest for mRest");
								resetTimes.add(sv);
					...
					sv.addTime(((MusicXMLForward) element).getTime()); // actually, time is the duration
				} else if (element instanceof MusicXMLBackup) {
					sv.substractTime(((MusicXMLBackup) element).getTime()); // actually, time is the duration
				} else {
	...
	protected void postProcess() throws ImportException, IM3Exception {
		...
		if (!song.getStaves().isEmpty()) {
			...
			if (ts == null) {
				Logger.getLogger(MusicXMLSAXImporter.class.getName()).log(Level.WARNING, "No meter found, inserting 4/4");
				ts = new FractionalTimeSignature(4, 4);
				firstStaff.addTimeSignature(ts);
			}
			if (ks == null) {
				Logger.getLogger(MusicXMLSAXImporter.class.getName()).log(Level.WARNING, "No key signature found, inserting C Major");
