...
public class AtomTest {
	...
	public void testSimpleConstructsInVoice() throws IM3Exception {
		ScoreLayer voice = new ScoreLayer(null, null, 1);
		...
		SimpleRest rest = new SimpleRest(Figures.EIGTHEEN, 0);
		voice.add(rest);
	...
	public void tieNoteTest() throws IM3Exception {
		...
		List<PlayedNote> pn = note.computePlayedNotes();
	...
	public void tiedChordsTest() throws IM3Exception {
		...
		List<PlayedNote> pn = chord.computePlayedNotes();
		assertEquals(3, pn.size());
		for (PlayedNote playedNote : pn) {
